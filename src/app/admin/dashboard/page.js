  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isRegister) {
        ...
        // REDIRECTION INSCRIPTION CORRIGÉE
        // Les comptes payants (DJ ou Solo) vont choisir leur pack. Les comptes gratuits vont direct sur /admin
        ...
        if (userDoc.exists()) {
          ...
          // REDIRECTION CONNEXION CORRIGÉE
          // Si c'est un compte payant qui n'a pas encore de pack -> plan-selection
          // Sinon tout le monde va sur /admin (où ils verront la liste de leurs soirées)
  ...
  return (
    <main className="min-h-screen bg-black flex items-center justify-center p-6 font-sans relative overflow-hidden text-white">
      ...
      <div className="glass-card w-full max-w-xl p-10 rounded-[40px] border border-white/5 relative z-10 bg-white/[0.02] backdrop-blur-3xl shadow-2xl">
        ...
        <button onClick={() => setIsRegister(!isRegister)} className="w-full mt-8 text-gray-600 text-[10px] uppercase font-black tracking-widest no-underline italic bg-transparent border-none cursor-pointer hover:text-white transition-colors">
