...
const params = { get: (key) => typeof window === 'undefined' ? null : new URLSearchParams(window.location.search).get(key) };
...
export default function AdminEmailPage() {
  const router = useRouter(); const [to, setTo] = useState(''); const [promo, setPromo] = useState(false);
  useEffect(() => { const p = new URLSearchParams(window.location.search); setTo(p.get('to') || ''); setPromo(p.get('promo') === '1'); }, []);
  const [subject, setSubject] = useState('');
  const [code, setCode] = useState('');
  const [message, setMessage] = useState('');
  useEffect(() => { if (promo) { setSubject('Bienvenue chez PartyLens — votre offre privilégiée'); setMessage(promoText('')); } }, [promo]);
